...
public interface DeviceMapper {
    ...
    @Delete({"DELETE FROM deviceGroups WHERE groupId = #{groupId}"})
    void deleteDeviceGroupsByGroupId(@Param("groupId") Integer groupId);

    @Insert({"<script>INSERT INTO deviceGroups (deviceId, groupId) VALUES " +
            "<foreach item='deviceId' collection='deviceIds' separator=','>(#{deviceId}, #{groupId})</foreach></script>"})
    void insertGroupDevices(@Param("groupId") Integer groupId, @Param("deviceIds") List<Integer> deviceIds);

    @Select({"SELECT devices.number FROM devices " +
            "INNER JOIN deviceGroups ON deviceGroups.deviceId = devices.id " +
            "WHERE deviceGroups.groupId = #{groupId}"})
    List<String> listDeviceNumbersByGroupId(@Param("groupId") Integer groupId);

    @Select({"SELECT deviceId FROM deviceGroups WHERE groupId = #{groupId}"})
    List<Integer> listDeviceIdsByGroupId(@Param("groupId") Integer groupId);
