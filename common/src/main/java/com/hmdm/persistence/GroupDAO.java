...
public class GroupDAO extends AbstractDAO<Group> {
    ...
    /** Full-replace a group's device membership (delete-then-insert, same idiom Configurations'
     *  app list already uses on save). An empty {@code deviceIds} just clears membership. */
    public void setGroupDevices(Integer groupId, List<Integer> deviceIds) {
        this.mapper.deleteDeviceGroupsByGroupId(groupId);
        if (deviceIds != null && !deviceIds.isEmpty()) {
            this.mapper.insertGroupDevices(groupId, deviceIds);
        }
    }

    /** Device numbers (the agent-v1 identifier) currently in this group — for queuing commands. */
    public List<String> listDeviceNumbersByGroupId(Integer groupId) {
        return this.mapper.listDeviceNumbersByGroupId(groupId);
    }

    /** Device ids currently in this group — for the console's device-picker editor state. */
    public List<Integer> listDeviceIdsByGroupId(Integer groupId) {
        return this.mapper.listDeviceIdsByGroupId(groupId);
    }
