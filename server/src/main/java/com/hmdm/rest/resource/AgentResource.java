...
public class AgentResource {
    ...
    /** Event type reported when the agent auto-suspends a non-MDM-initiated install pending
     *  admin approval (detail = package name). See proto/registry.md § Events. */
    private static final String EVENT_APP_INSTALL_PENDING = "appInstallPending";
    ...
    public Response checkin(@HeaderParam("Authorization") String authorization,
        ...
        if (request.getEvents() != null) {
            ...
            for (com.hmdm.rest.json.agent.AgentTelemetryEvent e : request.getEvents()) {
                ...
                if (EVENT_APP_INSTALL_PENDING.equals(e.getType()) && detail != null && !detail.trim().isEmpty()) {
                    commandDAO.insertPendingApprovalIfNotOpen(deviceNumber, detail.trim(), ts);
                }
            }
        }
