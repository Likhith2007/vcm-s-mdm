...
import com.mdmesh.core.command.handlers.AppApproveInstallHandler
...
import com.mdmesh.core.sync.PendingInstallScanner
import com.mdmesh.core.install.InstallManager
import com.mdmesh.core.install.InstalledPackagePoller
...
import com.mdmesh.core.install.SelfInitiatedInstalls
import com.mdmesh.policy.CapabilityRegistry
import com.mdmesh.policy.app.SelfInitiatedTracker
...
object AgentModule {
    ...
    fun provideCapabilityRegistry(
        handle: DpmHandle,
        selfInitiated: SelfInitiatedInstalls,
    ): CapabilityRegistry =
        // Adapts :core's SelfInitiatedInstalls to :policy's SelfInitiatedTracker interface (policy
        // can't depend on core) so appBlock/appHide's un-hide calls can mark the package before
        // Android re-fires ACTION_PACKAGE_ADDED for it — see SelfInitiatedTracker's doc comment.
        CapabilityRegistry(handle, SelfInitiatedTracker { pkg -> selfInitiated.mark(pkg) })
    ...
    /** Reliable check-in-driven fallback for non-MDM installs (see PendingInstallScanner). */
    @Provides
    fun providePendingInstallScanner(poller: InstalledPackagePoller): PendingInstallScanner =
        PendingInstallScanner { poller.poll() }
    ...
    @Provides
    @IntoSet
    fun provideAppApproveInstallHandler(installManager: InstallManager): CommandHandler =
        AppApproveInstallHandler(installManager)
